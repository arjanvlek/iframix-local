...
class MediaMixin:
    ...
    def handle_del_media(self, body):
        ...
        # Remove the source files synchronously, before responding: the
        # 2.3.4 display app refetches mediaList ~200 ms after delMedia
        # returns, and mediaList scans these directories — deleting in
        # the background would let that refetch briefly resurrect the
        # deleted photos. Cache/metadata cleanup and the MQTT publish
        # don't affect listing consistency, so they stay off the
            ...
            for filename, filepath in scan_photos(directory):
                ...
                if mid in media_ids:
                    try:
                        ...
                        logger.exception(
                            "[DEL MEDIA] could not remove %s", filepath)
                        continue
                    deleted.append((media_type, filename, mid))

        logger.info(
            "[DEL MEDIA] device=%s requested=%d deleted=%s",
            device_id, len(media_ids), [f for _, f, _ in deleted])
        self.respond_success(True)

        def _cleanup():
            deleted_ids = []
            for media_type, filename, mid in deleted:
